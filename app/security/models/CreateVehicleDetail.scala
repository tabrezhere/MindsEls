...
case class CreateVehicleDetail (
    id : Long, 
    Vehicle_no : String, 
    Vehicle_code : String,
    No_of_Seat : Int, 
...
object CreateVehicleDetail {
  
  implicit val reads: Reads[CreateVehicleDetail] = (
    (JsPath \ "id").read[Int] ~
    ...
    (JsPath \ "campusId").read[Long])(CreateVehicleDetail.apply( _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _))
