...
case class GuardianStudentDetails(
  id: Long,
  email: String,
  ...
  orgId : Long,
  orgName : String,
  studentDetailsList : ListBuffer[StudentUser]
) 


object GuardianStudentDetails {
  
  implicit val guardianStudentDetailsWrites = new Writes[GuardianStudentDetails] {
    def writes(guruser: GuardianStudentDetails): JsValue = Json.obj(
      "id" -> guruser.id,
      "email" -> guruser.email,
      ...
      "studentDetailsList" -> guruser.studentDetailsList)
  }
  
   implicit val reads: Reads[GuardianStudentDetails] = (
    (__ \ "id").read[Long] ~
    ...
    (__ \ "studentDetailsList").read[ListBuffer[StudentUser]])(GuardianStudentDetails.apply( _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _))
